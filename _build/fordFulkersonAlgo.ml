...
let add_arc_without_erase gr id1 id2 lbl =
  let current_outarcs = Graph.out_arcs gr id1 in
  let new_outarcs = List.append [(id2, lbl)] current_outarcs in
	let rec rem old_gr id new_gr = match old_gr with
      | [] -> new_gr
      | (ourIdToRemove, _)::tail when ourIdToRemove = id -> rem tail id new_gr
      | (notOurIdToRemove, out)::tail -> rem tail id ((notOurIdToRemove, out)::new_gr)
  in
  (* in le graph dans lequel on enlève les arcs de id1 ... *)
  let new_gr = rem gr id1 [] in
...
let residual_graph gr = Printf.printf "residual_graph-%!";
	...
	let rec f acu id out_arcs = match out_arcs with
		...
		| [] -> acu
		| (idD,{flow = 0; capacity})::tail -> Printf.printf "r1";f (add_arc acu id idD capacity) id tail
