...
(* Fonction d'export d'un graph en .dot *)
let export path graph =

  (* Open a write-file. *)
  let fichier = open_out path in

  (* Write in this file. *)
  fprintf fichier "digraph flowGraph {\n
		rankdir=LR;\n
		size=\"8,5\"\n
		node [shape=circle];\n" ;

  (* Write graph *)
  v_iter graph (fun idO out -> List.iter (fun (idD, lbl) -> fprintf fichier "%s -> %s [label = \"%s\"];\n" idO idD lbl) out) ;
